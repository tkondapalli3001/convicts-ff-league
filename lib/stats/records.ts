...
import { buildConsolationGameKeys, excludeManualGames, gameKey } from './game-filters'

export type RecordsData = ReturnType<typeof computeRecords>

type RecordsState = Pick<
  LeagueState,
  'allMatchups' | 'ownerSeasons' | 'brackets' | 'rosterUserMaps' | 'leagues'
>

/**
 * All-time record-book computations: scoring extremes, season extremes,
 * streaks, and rivalry counts. Pure — usable from React (useRecordsData)
 * and from the search resolvers.
 */
export function computeRecords(state: RecordsState) {
  const { allMatchups, ownerSeasons } = state

  const consolationGameKeys = buildConsolationGameKeys(state)
  const filteredMatchups = excludeManualGames(allMatchups)
  ...
  const low80   = validScores.filter(s => s.pts <= 80 && !consolationGameKeys.has(gameKey(s.year, s.week, s.owner, s.opp))).sort((a, b) => a.pts - b.pts)
